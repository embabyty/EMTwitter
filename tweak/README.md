...
> [!NOTE]  
> <b>EMTwitter custom sign-in</b><br>EMTwitter replaces the stock sign-in screen with a custom xAuth password login, which bypasses the Attestation block. Google and Apple sign-in are not supported (add a password to your account first). Two-factor authentication is handled through the in-app web challenge.</b>

> [!NOTE]  
> <b>EMTwitter floating navigation bar</b><br>EMTwitter replaces the stock tab bar with a floating frosted-glass navigation pill. It respects the tabs enabled in the Custom Tab Bar settings and uses Material 3-style icons (outlined when unselected, filled with the accent color when selected).</b>

> [!NOTE]  
> <b>EMTwitter custom logo</b><br>EMTwitter replaces the home-page top bar icon and the opening (launch) screen logo with the custom "M" logo, tinted with the current accent color. The home-page icon follows the "Color Twitter icon in top bar" setting.</b> 
