...
#import "EMPremium/EMPremiumViewController.h"
...
// EMTwitter Premium paywall forward declarations
static void BHT_showPremiumPaywallIfNeeded(void);
static void BHT_presentPremiumPaywall(void);
static BOOL BHT_isLoggedIn(void);
static BOOL BHT_isPremiumPaywallPresented(UIViewController *root);

// EMTwitter custom logo forward declarations
static UIImage *EMTwitterLogoImage(void);
static NSHashTable *EMLogoImageViews(void);
static void EMRefreshLogoColors(void);
static void EMRefreshLogoColorsDebounced(void);
static BOOL EMIsLaunchScreenImageView(UIView *view);
static void EMApplyLogoToImageView(UIImageView *iv);
static void EMFindAndReplaceLaunchScreenLogos(void);
...
// ===== EMTwitter Premium paywall =====
// Once the user is logged in (home timeline shown), a full-screen,
// non-dismissible paywall blocks the app until EMTwitter Premium is
// unlocked with a whitelisted Patreon email. See BHTManager for the
// whitelist and the Patreon URL.

// Best-effort check for "signed in": signed-out users get the
// ONBSignedOutViewController as (part of) their root view controller, or the
// custom LegacyLoginViewController (which must also count as signed-out,
// otherwise the premium paywall would cover the login form).
static BOOL BHT_isLegacyLoginScreen(UIViewController *vc) {
    if (!vc) return NO;
    if ([vc isKindOfClass:%c(LegacyLoginViewController)]) return YES;
    if ([vc isKindOfClass:[UINavigationController class]]) {
        UIViewController *first = ((UINavigationController *)vc).viewControllers.firstObject;
        if (first && [first isKindOfClass:%c(LegacyLoginViewController)]) return YES;
    }
    return NO;
}
...
static BOOL BHT_isLoggedIn(void) {
    ...
    if (!root) return NO;
    if ([root isKindOfClass:objc_getClass("ONBSignedOutViewController")]) return NO;
    if ([root isKindOfClass:[UINavigationController class]]) {
        UIViewController *visible = ((UINavigationController *)root).visibleViewController;
        if (visible && [visible isKindOfClass:objc_getClass("ONBSignedOutViewController")]) return NO;
    }
    for (UIViewController *vc = root; vc; vc = vc.presentedViewController) {
        if (BHT_isLegacyLoginScreen(vc)) return NO;
    }
    return YES;
}
...
static BOOL BHT_isPremiumPaywallPresented(UIViewController *root) {
    UIViewController *vc = root;
    while (vc) {
        if ([vc isKindOfClass:%c(EMPremiumViewController)]) return YES;
        vc = vc.presentedViewController;
    }
    return NO;
}
...
static void BHT_presentPremiumPaywall(void) {
    // The paywall covers the whole screen; drop the floating nav pill so it
    // doesn't linger behind the modal.
    [EMFrostedNavBar removeAll];

    if ([BHTManager Premium]) return;
    ...
    UIViewController *root = window.rootViewController;
    if (!root) return;
    if (BHT_isPremiumPaywallPresented(root)) return;

    UIViewController *host = BHT_topViewController(root);

    EMPremiumViewController *premium = [[EMPremiumViewController alloc] init];
    premium.modalPresentationStyle = UIModalPresentationFullScreen;
    if ([premium respondsToSelector:@selector(setModalInPresentation:)]) {
        premium.modalInPresentation = YES;
    }

    if (host.presentedViewController == nil) {
        [host presentViewController:premium animated:NO completion:nil];
    } else {
        [host dismissViewControllerAnimated:NO completion:^{
            UIViewController *newTop = BHT_topViewController(root);
            [newTop presentViewController:premium animated:NO completion:nil];
        }];
    }
}
...
static void BHT_showPremiumPaywallIfNeeded(void) {
    if ([BHTManager Premium]) {
        // Premium is unlocked (or was just unlocked): make sure the floating
        // nav pill is back in place.
        [EMFrostedNavBar installIfNeeded];
        return;
    }
    BHT_presentPremiumPaywall();
}
...
- (_Bool)application:(UIApplication *)application didFinishLaunchingWithOptions:(id)arg2 {
    ...
    // EMTwitter custom logo: replace the launch (opening) screen logo with
    // our logo tinted with the accent color, right after launch while the
    // launch screen is still visible.
    dispatch_async(dispatch_get_main_queue(), ^{
        EMFindAndReplaceLaunchScreenLogos();
    });
...
- (void)applicationDidBecomeActive:(id)arg1 {
    ...
    // EMTwitter Premium hard paywall: once logged in, block usage until
    // EMTwitter Premium is unlocked with a whitelisted Patreon email.
    if (![BHTManager Premium] && BHT_isLoggedIn()) {
        BHT_showPremiumPaywallIfNeeded();
    }

    // EMTwitter floating nav pill: reinstall whenever the app becomes active,
    // unless the premium paywall is up (it would cover it anyway).
    UIWindow *activeWindow = BHT_activeKeyWindow();
    if ([BHTManager Premium] ||
        !activeWindow || !BHT_isPremiumPaywallPresented(activeWindow.rootViewController)) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [EMFrostedNavBar installIfNeeded];
        });
    }

    // EMTwitter custom logo: re-tint with the current accent color.
    EMRefreshLogoColors();
    // EMTwitter floating nav pill: re-render icons with the current accent.
    [EMFrostedNavBar refreshTint];
}
...
- (void)viewDidAppear:(BOOL)animated {
    %orig(animated);
    // EMTwitter floating nav pill: install whenever the tab bar comes up.
    dispatch_async(dispatch_get_main_queue(), ^{
        [EMFrostedNavBar installIfNeeded];
    });
}
...
// MARK: - Override the login screens (EMTwitter custom xAuth sign-in)
...
- (void)_t1_showPremiumUpsellIfNeeded {
    // EMTwitter Premium paywall replaces Twitter's own premium upsell.
    if (![BHTManager Premium]) {
        BHT_showPremiumPaywallIfNeeded();
        return;
    }
...
// Older (non-THF) home timeline variants, for wider app-version coverage.
%hook T1HomeTimelineContainerViewController
- (void)_t1_showPremiumUpsellIfNeeded {
    if (![BHTManager Premium]) {
        BHT_showPremiumPaywallIfNeeded();
        return;
...
// MARK: - EMTwitter custom logo (stored as EMTwitterLogo.png in BHTwitter.bundle)
// The home-page top bar logo and the launch (opening) screen logo are replaced
// with this image, rendered as a template and tinted with the app's accent
// color so it follows accent color changes.

static UIImage *EMTwitterLogoImage(void) {
    static UIImage *logo = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSString *path = [[[BHTBundle sharedBundle] mainBundle] pathForResource:@"EMTwitterLogo"
                                                                         ofType:@"png"];
        if (path) {
            logo = [[UIImage imageWithContentsOfFile:path]
...
// Image views we replaced, kept weakly so we can re-tint them when the accent
// color changes.
static NSHashTable *EMLogoImageViews(void) {
...
static void EMRefreshLogoColors(void) {
    UIColor *accent = BHTCurrentAccentColor();
    for (UIImageView *iv in [EMLogoImageViews() allObjects]) {
        iv.tintColor = accent;
    }
}

static BOOL EMRefreshLogoColorsScheduled = NO;
static void EMRefreshLogoColorsDebounced(void) {
    if (EMRefreshLogoColorsScheduled) return;
    EMRefreshLogoColorsScheduled = YES;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)),
                   dispatch_get_main_queue(), ^{
                       EMRefreshLogoColorsScheduled = NO;
                       EMRefreshLogoColors();
                   });
}

// Launch screen images live inside a private "UILaunchStoryboardView".
static BOOL EMIsLaunchScreenImageView(UIView *view) {
    UIView *current = view.superview;
    while (current) {
        if ([NSStringFromClass([current class]) containsString:@"Launch"]) {
            return YES;
        }
...
// Replace the logo on one image view with the EMTwitter logo (template + accent).
static void EMApplyLogoToImageView(UIImageView *iv) {
    if (!iv || !iv.image) return;
    // Already replaced (our logo is template).
    if (iv.image.renderingMode == UIImageRenderingModeAlwaysTemplate) return;
    UIImage *logo = EMTwitterLogoImage();
    if (!logo) return;
    iv.image = logo;
    iv.tintColor = BHTCurrentAccentColor();
    [EMLogoImageViews() addObject:iv];
}

// Recursively find and replace logo images inside the launch screen view.
static void EMApplyLaunchScreenLogos(UIView *view) {
    if (!view) return;
    if ([view isKindOfClass:[UIImageView class]]) {
        EMApplyLogoToImageView((UIImageView *)view);
    }
    for (UIView *subview in view.subviews) {
        EMApplyLaunchScreenLogos(subview);
    }
}

// One-shot sweep: the launch (opening) screen is still on screen right after
// the app finishes launching, so find its view and swap in our logo.
static void EMFindAndReplaceLaunchScreenLogos(void) {
    for (UIWindow *w in [UIApplication sharedApplication].windows) {
        for (UIView *v in w.subviews) {
            if ([NSStringFromClass([v class]) containsString:@"Launch"]) {
                EMApplyLaunchScreenLogos(v);
            }
        }
    }
}
...
- (void)setImage:(UIImage *)image {
    ...
    // Our logo is already applied (template mode) - prevents recursion.
    if (image.renderingMode == UIImageRenderingModeAlwaysTemplate) return;

    // Opening/launch screen logo: always replace with the EMTwitter logo.
    if (EMIsLaunchScreenImageView(self)) {
        EMApplyLogoToImageView(self);
        return;
    }

    // If the setting is off, keep the original behavior.
    if (!BHColorTwitterIconEnabled()) return;
    ...
    if ([image respondsToSelector:@selector(tfn_dynamicColorImageName)]) {
        ...
        if ([imageName isEqualToString:@"twitter"]) {
            // Home-page top bar logo: replace with the EMTwitter logo.
            UIImage *logo = EMTwitterLogoImage();
            if (logo) {
                EMApplyLogoToImageView(self);
                return;
            }
            // Fallback: tint the original bird with the accent color.
...
// The launch screen's image views may get their image before they are added to
// the view hierarchy, so also catch them when they move into the window.
- (void)didMoveToWindow {
    %orig;
    if (self.window && EMIsLaunchScreenImageView(self)) {
        EMApplyLogoToImageView(self);
    }
}
...
%ctor {
    ...
    // Re-tint the EMTwitter logo and nav pill whenever the accent/theme color
    // changes (accent selections are persisted in NSUserDefaults) or the app
    // becomes active again.
    [[NSNotificationCenter defaultCenter] addObserverForName:NSUserDefaultsDidChangeNotification
                                                    object:nil
                                                     queue:[NSOperationQueue mainQueue]
                                                usingBlock:^(NSNotification * _Nonnull note) {
        EMRefreshLogoColorsDebounced();
        [EMFrostedNavBar refreshTint];
    }];
