...
// Patreon page users are sent to in order to unlock EMTwitter Premium.
+ (NSString *)premiumPatreonURL {
    return @"https://www.patreon.com/cw/EmAppleFlagship";
}
// Patreon emails that automatically unlock EMTwitter Premium.
// The app cannot verify Patreon subscriptions itself, so this is a
// hardcoded whitelist. Add more emails to this array to whitelist them.
+ (NSArray *)premiumEmails {
    return @[
        @"ios11emiry@gmail.com"
    ];
}

+ (BOOL)Premium {
    return [[NSUserDefaults standardUserDefaults] boolForKey:@"em_premium_unlocked"];
}
+ (void)setPremium:(BOOL)unlocked {
    [[NSUserDefaults standardUserDefaults] setBool:unlocked forKey:@"em_premium_unlocked"];
}
+ (NSString *)premiumEmail {
    return [[NSUserDefaults standardUserDefaults] stringForKey:@"em_premium_email"];
}
+ (void)setPremiumEmail:(NSString *)email {
    [[NSUserDefaults standardUserDefaults] setObject:email forKey:@"em_premium_email"];
}
+ (BOOL)isPremiumEmail:(NSString *)email {
    ...
    for (NSString *allowed in emails) {
        if ([lower isEqualToString:[allowed lowercaseString]]) return YES;
    }
    return NO;
}
